...
def render_debug_page():
    ...
    if not st.session_state["dev_authenticated"]:
        ...
        with c2:
            st.markdown("""
            <div style='border: 1px solid rgba(128, 128, 128, 0.2); border-radius: 12px; padding: 2.5rem; text-align: center; margin-top: 3rem;'>
    ...
    left_menu, right_content = st.columns([1, 4])
    
    with left_menu:
        st.markdown("<h4 style='color: #00A0DC; margin-bottom: 1rem;'>Console Menu</h4>", unsafe_allow_html=True)
...
def render_cv_builder():
    ...
    # STEP 4: REVIEW & EXPORT
        ...
        # Build clean text for export (Simple & Reliable)
        cv_text_lines = []
        if cv_data.get("name"):
            cv_text_lines.append(cv_data["name"].upper())
        ...
        if cv_data.get("experiences"):
            ...
            for exp in cv_data["experiences"]:
                if exp.get("title"):
                    header = f"{exp['title']}"
                    if exp.get("company"): header += f" @ {exp['company']}"
                    if exp.get("dates"): header += f" ({exp['dates']})"
                    cv_text_lines.append(header)
                    if exp.get("bullets"): cv_text_lines.append(exp["bullets"])
        
        if cv_data.get("education"):
            cv_text_lines.append("\nEDUCATION")
            for edu in cv_data["education"]:
                header = f"{edu.get('degree', '')}"
                if edu.get("institution"): header += f" @ {edu['institution']}"
                if edu.get("dates"): header += f" ({edu['dates']})"
                cv_text_lines.append(header)
        ...
        with st.container(border=True):
            ...
            st.markdown("""
                <div style='background: rgba(0, 160, 220, 0.05); padding: 1.5rem; border-radius: 10px; border: 1px solid rgba(0, 160, 220, 0.2); margin-bottom: 1.5rem;'>
                    <p style='margin: 0; color: #c9d1d9; font-size: 0.95rem;'>
                        The export includes all your professional details, skills, and experience in an <b>ATS-optimized format</b>.
                    </p>
                </div>
            """, unsafe_allow_html=True)
            
            col1, col2 = st.columns(2)
            candidate_name = cv_data.get("name", "My_CV").replace(" ", "_")
            ...
            st.divider()
            if st.button("← Back to Edit Skills & Experience", use_container_width=True):
                st.session_state["cv_builder_step"] = 3
                st.rerun()
...
def render_landing_page():
    ...
    <div style='text-align: center; margin-bottom: 2.5rem;'>
        <h2 style='font-size: 2rem; font-weight: 700; margin-bottom: 0.3rem;'>{t('why_title')}</h2>
        <p style='color: var(--text-secondary); font-size: 1rem;'>{t('why_subtitle')}</p>
    </div>
    """, unsafe_allow_html=True)
    ...
    with d1:
        ...
        <div style='background: rgba(0, 160, 220, 0.05); border: 1px solid rgba(0, 160, 220, 0.2); border-radius: 12px; padding: 1.8rem; margin-bottom: 1.5rem; min-height: 220px;'>
            <div style='font-size: 2.8rem; font-weight: 800; color: #00A0DC; margin-bottom: 0.3rem;'>{t('why_card1_val')}</div>
    ...
    with d2:
        ...
        <div style='background: rgba(0, 119, 181, 0.05); border: 1px solid rgba(0, 119, 181, 0.2); border-radius: 12px; padding: 1.8rem; margin-bottom: 1.5rem; min-height: 220px;'>
            <div style='font-size: 2.8rem; font-weight: 800; color: #0077B5; margin-bottom: 0.3rem;'>{t('why_card2_val')}</div>
            <div style='font-size: 0.95rem; color: #c9d1d9; line-height: 1.5; margin-bottom: 0.8rem;'>
                {t('why_card2_text')}
            </div>
    ...
    with d3:
        ...
        <div style='background: rgba(229, 57, 53, 0.05); border: 1px solid rgba(229, 57, 53, 0.2); border-radius: 12px; padding: 1.8rem; margin-bottom: 1.5rem; min-height: 220px;'>
            <div style='font-size: 2.8rem; font-weight: 800; color: #E53935; margin-bottom: 0.3rem;'>{t('why_card3_val')}</div>
            <div style='font-size: 0.95rem; color: #c9d1d9; line-height: 1.5; margin-bottom: 0.8rem;'>
                {t('why_card3_text')}
            </div>
    ...
    with d4:
        ...
        <div style='background: rgba(0, 200, 83, 0.05); border: 1px solid rgba(0, 200, 83, 0.2); border-radius: 12px; padding: 1.8rem; margin-bottom: 1.5rem; min-height: 220px;'>
            <div style='font-size: 2.8rem; font-weight: 800; color: #00C853; margin-bottom: 0.3rem;'>{t('why_card4_val')}</div>
            <div style='font-size: 0.95rem; color: #c9d1d9; line-height: 1.5; margin-bottom: 0.8rem;'>
                {t('why_card4_text')}
            </div>
    ...
    <div style='background: rgba(0, 68, 113, 0.1); border: 1px solid rgba(0, 119, 181, 0.25); border-radius: 10px; padding: 1.2rem 1.5rem; text-align: center; margin-bottom: 1.5rem;'>
        <p style='margin: 0; font-size: 0.95rem; color: #c9d1d9;'>
            <strong style='color: #00A0DC;'>{t('why_summary_bold')}</strong>{t('why_summary_text')}
        </p>
    </div>
    """, unsafe_allow_html=True)
...
def render_interview_prep():
    """
    INTERVIEW PREP PAGE - Premium Design
    """
    render_navigation()
    ...
    if not questions:
        ...
        # Practice Mode - Minimalist Single Column Flow
        q_idx = st.session_state.get("current_q_index", 0)
        current_q = questions[q_idx]
        
        # 1. Clean Progress Indicator
        ...
        # Center Container for focus
        with st.container():
            # Apply max-width via columns trick
